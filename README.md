...
LINE公式アカウントを友だち追加すると自動で登録され、操作ボタンが表示されます。
...
2. `開始` で監視開始
3. 指定日に空き（●/▲）が出現したら、その日付について通知
4. 同じ日付は空きが続いている間は再通知しない
5. 空きがなくなり、再度出現したら再通知
6. 監視日が過去日になると自動で削除され、**すべて過去日になったら監視を自動停止**して通知（通知カードから日付を選び直せます）
...
- `watch/state`: 最新チェック結果（`availableDates` が前回空きのあった日付）

### テスト通知（手動）

Firebase Console → Functions → `watchScheduler` → 「Run in Shell」

または、Firestoreで `watch/state.availableDates` を空配列に、`watch/config.enabled` を `true` にして、空きが出た状態をシミュレート。
...
> `availableDates` は前回チェック時点で空きがあり、**かつ通知に成功した**日付。ここに**含まれていない**日付に空きが出たときだけ通知するため、日付ごとに1回だけ通知される。Push に失敗した日付は記録されないため、次回チェックで再送される。
